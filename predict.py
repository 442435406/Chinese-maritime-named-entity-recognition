...
model.load_state_dict(torch.load(MODEL_DIR + 'bert\\' + '280_MSAR_NER_1e-05_8_0.3_30.pth'))
model.to(DEVICE)

# BiLSTM
model_1 = MSAR_NER(CURRENT_PLM) # 在model模块中再写一个用作对比的模型类，如MSAR_NER_nobert
model_1.load_state_dict(torch.load(MODEL_DIR + 'bert\\' + '280_MSAR_NER_1e-05_8_0.3_30.pth'))
model_1.to(DEVICE)
...
if __name__ == "__main__":
    ...
    if os.path.exists(COMPARE_RESULT_DIR) is True:
        ...
        with open(COMPARE_RESULT_DIR, 'w', encoding='utf-8') as f:
            f.write('text label bert bilstm\n')
            for i in range(0,len(words)):
                for j in range(0,len(words[i])):
                    f.write(words[i][j] + ' ' + Y[i][j] + ' ' + Y_hat[i][j] + ' ' + Y_hat_1[i][j] + '\n')
            f.write('\n\n')
        print('Finished!')
